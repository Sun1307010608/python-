...
def get_start_arrive_station(s):
    while True:
        ...
        if from_station in s:
            from_station_mark = s[from_station]
            break
        else:
            print('您输入的出发站地点不存在!!!')
            continue
    ...
    while True:
        ...
        if to_station in s:
            to_station_mark = s[to_station]
            break
        else:
            print('您输入的目的站不存在!!!')
            continue
    return from_station_mark,to_station_mark
...
def get_ticket_info(train_date,from_station_mark,to_station_mark):
    url = "https://kyfw.12306.cn/otn/leftTicket/query?leftTicketDTO.train_date=%s&leftTicketDTO.from_station=%s&leftTicketDTO.to_station=%s&purpose_codes=ADULT" %(train_date, from_station_mark, to_station_mark) 
    ##print(url)
    while True:
        try:
            html = get_html(url)
...
def print_ticket_info(train_list):
    if train_list:
        ...
        for train in train_list:
            for index,c in enumerate(train):
                if not c:
                    train[index] = '--'
            x.add_row(train)
        print(x)
...
def get_train_list():
    ...
    s = get_stations()
    # 根据输入的车站得到其代号
    from_station_mark,to_station_mark = get_start_arrive_station(s)
    # 获取用户输入的出发日期
    train_date = get_start_date()
    # 根据起始站,火车出发时间获取车次信息
    train_info = get_ticket_info(train_date,from_station_mark,to_station_mark)
    # 提取车次信息
